using HZJ.CommonCls.Image;
using iTextSharp.text;
using iTextSharp.text.pdf;
using System;

namespace HZJ.CommonCls.Pdf
{
    public class HeaderAndFooterEvent : PdfPageEventHelper, IPdfPageEvent
    {
        ...
        /// </summary>
        public static iTextSharp.text.Image AddImage(string path, string dPath, int Alignment, float marginRight, float marginBottom)
        {
            try
            {
                if (!ImageHelper.CompressImage(path, dPath, 80, 300, true))
                {
                    throw new Exception("图片添加出错");
                }

                iTextSharp.text.Image img = iTextSharp.text.Image.GetInstance(dPath);
